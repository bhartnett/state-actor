...
<p align="center">
  <strong>Generate client-ready Ethereum databases for geth, reth, besu, nethermind, ethrex, erigon, and nimbus &mdash; without going through each client's <code>init</code> path.</strong>
</p>
...
# cgo clients (besu / nethermind / reth / ethrex / erigon / nimbus) ship as prebuilt images:
docker pull ghcr.io/ethereum/state-actor-reth:main
```

`besu`, `nethermind`, `reth`, `ethrex`, `erigon`, and `nimbus` need cgo bindings (RocksDB / MDBX). The published `ghcr.io/ethereum/state-actor-<client>:main` images carry those bindings; pull the one you need (`state-actor-besu`, `state-actor-nethermind`, `state-actor-reth`, `state-actor-ethrex`, `state-actor-erigon`, `state-actor-nimbus`). To build locally instead, the per-client `Dockerfile.<client>` files ship in this repo.
...
docker build -f Dockerfile.nimbus     -t state-actor-nimbus:dev .      # cgo
...
Every client runs the same way: mount an output directory at `/data` and run its `ghcr.io/ethereum/state-actor-<client>:main` image. Substitute `geth` / `reth` / `besu` / `nethermind` / `ethrex` / `erigon` / `nimbus` for the client and the matching image.
...
Run `--list-forks` for accepted `--fork` values. The default fork is the latest one each `--client` supports (currently `osaka` across all seven).
...
| nimbus | [docs/RUNBOOK.md#nimbus](docs/RUNBOOK.md#nimbus) |
...
| `--client` | string | `geth` | Target client: `geth`, `nethermind`, `besu`, `reth`, `ethrex`, `erigon`, `nimbus` |
...
See [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) for the deep dive (writer phases, per-client format notes, the streaming-trie / streaming-sort packages). The short version: entity generation streams into a per-client writer that produces the client-native database directly, with cross-client determinism guaranteed by `internal/sizecal/`'s single global `bytesPerSlot` constant (identical across all seven clients by design).
...
CI matrix lives in `.github/workflows/ci.yml`. The cross-client `cross-client-genesis-root` job pins the invariant: same `--seed` + same spec → identical state root across all seven MPT clients.
